...
/* how many powers of 2's worth of buckets we use ,桶的大小为2^hashpower*/
unsigned int hashpower = HASHPOWER_DEFAULT;

#define hashsize(n) ((ub4)1<<(n))
#define hashmask(n) (hashsize(n)-1)

/* Main hash table. This is where we look except during expansion. 在hash表扩容时，会有次hash表*/
static item** primary_hashtable = 0;

/* Previous hash table. During expansion, we look here for keys that haven't been moved over to the primary yet. */
...
/* During expansion we migrate values with bucket granularity; this is how far we've gotten so far.
...
void assoc_init(const int hashtable_init)
{
    ...
    //hashpower的默认值为16,如果未设置新值，则按默认值进行初始化主hash表
...
//唤醒扩容线程assoc_maintenance_thread
...
/* 向hash表中添加元素。Note: this isn't an assoc_update.  The key must not already exist to call this */
...
void assoc_delete(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    if (*before) {
        item *nxt;
        hash_items--;
        /* The DTrace probe cannot be triggered as the last instruction due to possible tail-optimization by the
         * compiler */
    ...
    /* Note:  we never actually get here.  the callers don't delete things they can't find. */
    assert(*before != 0);
}
...
/*启动扩容线程，扩容线程在main函数中会启动，启动运行一遍之后会阻塞在条件变量maintenance_cond上面，插入元素超过规定，assoc_start_expand
会唤醒该条件变量 */
static void *assoc_maintenance_thread(void *arg)
{
    ...
    while (do_run_maintenance_thread)
    {
        ...
        if (!expanding)
        {
            ...
            assoc_expand();//申请新的hash表，primary_hashtable,用于下次扩容
            mutex_unlock(&cache_lock);
        }
    }
    return NULL;
...
int start_assoc_maintenance_thread()
{
    ...
    if (env != NULL)
    {
        hash_bulk_move = atoi(env);//hash表扩容的增量
        if (hash_bulk_move == 0)
        {
            hash_bulk_move = DEFAULT_HASH_BULK_MOVE;
        }
    }
    //创建一个线程，其入口函数为assoc_maintenance_thread
